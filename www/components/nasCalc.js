...
2025/04/19	ラップと表記していた機能が一般にいうスプリットタイムであったため新しく LAP|SPLITの切り替えを実装
モード変更の際はメモリが自動でリセット（クリア）されるので注意
...
//	変数初期化
var VER="25061401";
var autoFit = false;//ウィンドウに自動でフィットさせる
var calcConsole = {
	leftBuf  :"",
	centerBuf:"",
	rightBuf :""
...
var swMODE      = "split"     ;//計測モード split | lap
...
var checkMX=MaxLap;
	let LAP = new Array(MaxLap);
	let LapName = new Array(MaxLap);
		for(i = 0; i < MaxLap; i++){
			LapName[i] = 'lap' + i;
			LAP[i] = nas_FR2TC(0)[1] ;
...
//電卓のスタック
//スタックに push pop dup top rotation lock add sub div mulを登録
//push|popはArray本来のメソッドを使用
var operandStack       = new Array();
	operandStack.fixed = false;//スタック編集フラグ
operandStack.dup = function(){this.push(this.top());}
operandStack.exc = function(){
	if (this.length > 1) operandStack.splice(-2,2,...operandStack.slice(-2).reverse());
}
operandStack.top = function(){ return operandStack.slice(-1)[0];}
operandStack.rotationLeft  = function(){operandStack.push(operandStack.shift());}
operandStack.rotationRight = function(){operandStack.unshift(operandStack.pop());}
...
nas.FRATE.parse(newrate)	; RATE = newrate; pastrate = RATE;
...
//	document.getElementById("CHG.src="./images/02.png";
	document.getElementById("CHG").src="./images/02.png";
		break;
case "stopwatch":
	m_action=true ;
	nas_Push_Log("Change Mode stopwatch");
//	document.getElementById("CHG.src="./images/01.png";
	document.getElementById("CHG").src="./images/01.png";
		break;
...
//	document.getElementById("CHG_clock.value="CLOCK";
	document.getElementById("CHG_clock").src="./images/04.png";
		break;
case "watch":
//	m_action=false ;
	nas_Push_Log("Change Mode Stop-Watch");
//	document.getElementById("CHG_clock.value="WATCH";
	document.getElementById("CHG_clock").src="./images/03.png";
		break;
...
if(true){
	var SPLIT = nas_FR2TC(nas_cc2FR(cc - Start))[1];
	var LAP   = nas_FR2TC(nas_cc2FR(cc - Runner))[1];
	document.getElementById("nas_display").value = SPLIT;
//ラップを表示（コンソールは常時表示に仕様変更）
	putsCONS(((swMODE=='split')? SPLIT:LAP),'center');
}else{
//	従来のストップウオッチ用
...
default :       PreFix =""
	...
	nas_Push_Log(PreFix + "\t" + ((swMODE=='split')? SPLIT:LAP));
}
//	end proc
//	スタートとストップ引数なし
...
ClockClicks = new Date(); now =ClockClicks.getTime()
	...
	switch(STATUS) {
case "stop":
		Start  = now - (Stop - Start);//現時刻から開始オフセット更新
		if(Stop != 0) {nas_push_LAP(now)} else {nas_Push_Log("Start")}
		Runner = now;
		STATUS ="run";
		document.getElementById("SWButton").value = "= STOP =";
		document.getElementById("LapButton").value = (swMODE == 'lap')?"Lap":"Split";
		break;
case "run":     nas_cc2dp(now);
		Stop = now;
		Runner = Stop;
		STATUS = "stop";
		document.getElementById("SWButton").value = "= START =";
		document.getElementById("LapButton").value = "Reset";
		break;
case "run\-spl":
case "run\-lap":
		STATUS = "run";
...
case "stop":
		...
		Start = 0 ; Stop = 0 ; Runner = 0 ; nas_Push_Log("\treset\n");
		running = Ri[0];
...
case "run":
		nas_push_LAP(now);
		Runner = now;
		STATUS = "run-"+((swMODE == 'lap')? 'lap':'spl');
		document.getElementById("sTatus").innerHTML = STATUS;
		document.getElementById("SWButton").value = "= release =";
		break;
case "run\-spl":
case "run\-lap":
		nas_cc2dp(now);
...
//何はなくとも現在時を保持
ClockClicks = new Date();ct_K = ClockClicks.getTime();nas_capt = "Key";
//putsCONS(getKEYCODE(e)+'/ press /'+String.fromCharCode(getKEYCODE(e)));
...
//if (! document.getElementById("SIbuf").focus()) return true;
	switch(key) {
/*
//コントロールキーはpressイベントが発生しないため以下の判定は不要
case 9 :        ;//[TAB]
case 37 :       ;//[left]
case 38 :       ;//[up]
case 39 :       ;//[right]
...
case 3  :     ;//[enter](mac)
case 13 :     ;//[Return]
case 61 :    pushKey('='); break;//[=]
/*
144はnumlockだった
*/
case 37 :	pushKey('MOD');break;//[%]>MOD
case 94 :	pushKey('^')  ;break;//[^]>pow
//case 86 :	pushKey('MOD');break;//[V]>MOD
case 90 :	pushKey('delta');break;//[Z]>delta
case 109 :	pushKey('m2i');break;//[m]>mm2inch
//case 109 :	if(TC){pushKey('m')}else{pushKey('m2i')};break;//[i]>m2i
case 105 :	pushKey('i2m');break;//[i]>inch2mm
...
// case 113 :	nas_ChangeKBD('stopwatch');break;//[q]>入力モード切り換え
...
/*
	キーアップ処理
*/
function nas_InputU(e) {
//キーアップをキャプチャ
//何はなくとも現在時を保持
ClockClicks = new Date();ct_K = ClockClicks.getTime();nas_capt = "Key";
//putsCONS(getKEYCODE(e)+'/:Up/');
//putsCONS(nas_cc2FR(ct_K)+"_");
	if ( KeyMODE == 'calculator' ){
//電卓モード
		var key = getKEYCODE(e);//キーコードを取得
...
case 9  :       ;break;//[TAB]
case 127:       pushKey('CE'); break;//[del]
case 37 :       pushKey('exc'); break;//[left]
case 38 :       pushKey('pop'); break;//[up]
case 39 :       pushKey('dup'); break;//[right]
case 40 :       pushKey('MEM'); break;//[down]
//case 32 :       return true; break;//SPACE
...
// for IE
	Ob = event.srcElement.type ;Bt = event.button ;
	switch(Ob) {
	case "text":	;
	case "":	;
...
function nas_write_Log(){
	putsCONS("",'left');//左バッファクリア
	for(l=0; l<Log.length;l ++){putsCONS(Log[l],'left')}
}
...
*/
function putsCONS(string,alignment){
//コンソールが表示されていない場合は表示する。(常時表示に変更 2504)
//	if(document.getElementById("consoleBox").style.display=="none"){kb_Flip("cons");}
//LCR(left|center|right)指定された場合、バッファの切替を伴う場合がある
	if(string == undefined)string = '';
	var currentAlign = document.getElementById("CONS").style.textAlign;
	if(alignment){
//指定あり
		if(alignment != currentAlign){
//アンマッチ（切り替え）
			document.getElementById("CONS").style.textAlign = alignment;
			document.getElementById("CONS").value = calcConsole[alignment+"Buf"];;
		};
	}else{
		alignment = currentAlign;
	};
	var targetBuf = calcConsole[alignment+"Buf"];
	if(string == ""){
		targetBuf = "";
	}else{
		targetBuf = string + '\n' + targetBuf;
	};
	calcConsole[alignment+"Buf"] = targetBuf;
//文字を表示する
	document.getElementById("CONS").value = String(targetBuf);
}
//ステータス行にメッセージ表示
...
/*計測モード変更 Split | Lap
*/
function nas_ChangeSWMODE(mode) {
	if (mode == 'next'){
		mode = ["lap","split"][nas_Alsearch(["split","lap"],swMODE)];
	}
	swMODE = mode;
	document.getElementById("CHG_swMODE").innerHTML = ['Lap','Split'][nas_Alsearch(["lap","split"],swMODE)];
	putsCONS('','center');
}
...
function chgINPUTmode(mode) {
	if (mode == 'next'){
		mode = ["dec","fct","hex","oct","bin"][nas_Alsearch(["fct","hex","oct","bin","dec"],INPUTmode)];
	}
	INPUTmode=mode;
...
function chgResolution(vAlue){
	...
	nas.RESOLUTION.setValue(vAlue,'ppi') ;

	document.getElementById("Resolution").innerHTML=Math.floor(nas.Dpi()) + 'ppi';
	}
}
//電卓バッファに対する直接入力の処理
...
/*
	スタックの内容をスタックの先頭を除いてコンソールに出力
	スタックトップは入力バッファに表示されているので、スタックの長さが１の場合は表示なし
	
*/
function putsStack(){
	if(operandStack.length <= 1) {putsCONS("",'right');return;};
	var resUlt = [];
	for (num=operandStack.length-1;num>0;num--){
//モードごとにスタックの内容を文字列に変換
		if(INPUTmode == 'fct'){
			resUlt.push(nas_FR2TC(operandStack[num-1])[1]);
		}else{
			resUlt.push(operandStack[num-1].toString(cardinal));
		};
//		resUlt.push(operandStack[num-1]);
	};
//console.log(resUlt);
	putsCONS("",'right');
	putsCONS(resUlt.join('\n'));
...
break;
case		"rtR":	//メモリ?
	operandStack.push(operandStack.shift());
break;
case		"rtL":	//メモリ?
	operandStack.unshift(operandStack.pop());
break;
//単項演算
//case		"":	//
//	operandStack.push();
...
// コマンドの結果で表示を更新
if (dEbug) putsCONS( operandStack.top(),'left');
...
case 'cons':
	var currentAlign = document.getElementById("CONS").style.textAlign;
	var targetAlign  = ['center','right','left'][['left','center','right'].indexOf(currentAlign)];
	var targetBuf = calcConsole[targetAlign+"Buf"];
	document.getElementById('CONS').style.textAlign = targetAlign;
	document.getElementById('CONS').value = targetBuf
	break;

case 'kbd':
/*	キーボードレイアウト
	tall|wide|hide　をローテーションする仕様に変更
*/
	if(document.getElementById('kbdField').style.display=="none"){
		document.getElementById('kbdField').style.display="";
	}else{
		document.getElementById('kbdField').style.display="none";
	}
	break;
default	:	alert(aRgs);
	}
...
//	if (autoFit) nas_fitToWindow();
//		setTimeout("nas_fitToWindow();",50);
...
/*コンテンツをウインドウにフィットさせる関数、引数は特になし

baseFieldをスケーリングしてWindowにフィットさせる
要素は正比率でコンテンツの形状に合わせウインドウの比率を考慮してスケールを決定する

(ウインドウ(縦:横) > コンテンツ(縦:横))? 縦 : 横
*/
...
baseFieldのサイズを取得してWindowをフィットさせる
条件によりこの関数は働かないことが多いので廃止予定
*/
function nas_sizeToContent(){
//	if(! window.opener) return false;
//	if(! autoFit){return;}else{}
		var form = (document.getElementById('fkbBox').style.display=="none")? "box":((window.innerWidth/window.innerHeight >= 1)?
		"landscape":"portrate");//box|portrate|landscape
		return form;
//横サイズ
		var innerWidth = document.getElementById('baseField').getBoundingClientRect().width;
//縦サイズ
		var innerHeight = document.getElementById('baseField').getBoundingClientRect().height;
...
function handleTouchMove(evt){
	if(evt.target instanceof HTMLInputElement){
		console.log(evt);
//		if(evt.type == 'touchen)evt.target.classList.add()
	}
//	if(evt.target.id !='CONS') evt.preventDefault();
};
/*
	ボタン類のイベントハンドル
*/
...
return;	}
	document.body.addEventListener('pointerdown',function(evt){
		if(evt.target == document.body) nas_Capt('Start_Stop');
	})
...
//	document.addEventListener('touchmove', handleTouchMove, { passive: false });
	...
	chgResolution(document.getElementById("ppi").value);
